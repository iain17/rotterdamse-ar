...
class ARViewController: UIViewController, MKMapViewDelegate, SceneLocationViewDelegate{
    // This to not start ARKit too early and when is not needed
    ...
    override func viewDidLayoutSubviews() {
        ...
        if showMapView {
            ...
            infoLabel.frame.origin.y = self.view.frame.size.height - (infoLabel.frame.size.height + 80)
        }
        ...
        if let currentLocation = sceneLocationView.currentLocation() {
            DispatchQueue.main.async {
                ...
                if self.userAnnotation == nil {
                    ...
                    self.userAnnotation?.title = "My position"
                    self.userAnnotation?.subtitle = "GPS based"
    ...
    @objc func updateInfoLabel() {
        ...
        if let latitude = sceneLocationView.currentLocation()?.coordinate.latitude,
            let longitude = sceneLocationView.currentLocation()?.coordinate.longitude {
            infoLabel.text!.append("lat: \(latitude), long: \(longitude)\n")
        }
        
        if let horizontalAccuracy = sceneLocationView.currentLocation()?.horizontalAccuracy,
            let verticalAccuracy = sceneLocationView.currentLocation()?.verticalAccuracy {
            infoLabel.text!.append("H-Accuracy: \(Double(horizontalAccuracy)), V-Accuracy: \(Double(verticalAccuracy))\n")
        }
